...
let arr = [],
    id = 0;
let addUser = function(id, user) {
    let text = `<div id="` + arr[id].id + `" class="col-sm-6 col-md-4">
			<div class="thumbnail">
				<div class="caption">
...
let bindApply = function(id, user) {
    $('.apply').bind('click', function() {
        applyModification(id, user);
    })
}
...
var applyModification = function(id, user) {
    if (!($('#' + id + ' #name').val() && $('#' + id + ' #salary').val())) {
        ...
        let text = `<h3>` + arr[id].name + `<a id="` + arr[id].id + `" class='btn btn-default pull-right addToCompany'>+</a></h3>
						<p>Зарплата:` + arr[id].salary + `</p>
					   <p>`
        if (user == 'programmer') {
            text = text + `Язык программирования:` + (arr[id].language ? arr[id].language : 'не задано') + `</p>
					   <p>`
        }

        text = text + `Опыт работы:` + (arr[id].experience ? arr[id].experience : 'не задано') + `</p>
					   <p>
        ...
        $('#' + id + ' > .thumbnail > .caption').html(text)
        bindModify(id);
        bindDelete();
        bindAddToCompany();
    }
